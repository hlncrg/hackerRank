...
for caseIndex in range(numCases):
    ...
    ladders.append([[int(j) for j in raw_input().split(' ')] 
                   for i in range(numLadders[caseIndex])])
    numSnakes.append(int(raw_input()))
    snakes.append([[int(j) for j in raw_input().split(' ')] 
                  for i in range(numSnakes[caseIndex])])

ladders=[{j[0]:j[1] for j in i} for i in ladders]
snakes=[{j[0]:j[1] for j in i} for i in snakes]


    
def findBest(square,caseIndex,solution):
    if square>=100:#if the square is 100 or more you are there
        return 0
    for squareNew in range(square+1,square+7):#for each of the different dice rolls
        if squareNew not in solution.keys():#if you have not already found this solution
            if squareNew in snakes[caseIndex].keys():#if there is a snake at this location
                if snakes[caseIndex][squareNew] in solution.keys():#if the end of the snake solution has been found
                    if solution[snakes[caseIndex][squareNew]]!=-1:#if you are not already exploring this square
                        solution[squareNew]=solution[snakes[caseIndex][squareNew]]#the solution to the beginning of the snake is the same as the end
                    else:#otherwise explore the end of the snake
                        solution[squareNew]=-1
                        solution[squareNew]=findBest(snakes[caseIndex][squareNew],caseIndex,solution)
                else:#if the end end of the snake is not in the solutions
                    solution[squareNew]=-1#look for the solution
                    solution[squareNew]=findBest(snakes[caseIndex][squareNew],caseIndex,solution)
            elif squareNew in ladders[caseIndex].keys():#is this square the beginning of a ladder?
                if ladders[caseIndex][squareNew] in solution.keys():#is the end of the ladder in the solutions?
                    if solution[ladders[caseIndex][squareNew]]!=-1:#if you are not already exploring this square
                        solution[squareNew]=solution[ladders[caseIndex][squareNew]]#the solution at the beginning of the square is the same as the end
                    else:#Otherwise explore the end of the ladder
                        solution[squareNew]=-1
                        solution[squareNew]=findBest(ladders[caseIndex][squareNew],caseIndex,solution)
                else:#Otherwise explore the end of the ladder
                    solution[squareNew]=-1
                    solution[squareNew]=findBest(ladders[caseIndex][squareNew],caseIndex,solution)
            else:#if this is not the beginning of a ladder or snake
                solution[squareNew]=-1#what is the shortest path?
                solution[squareNew]=findBest(squareNew,caseIndex,solution)
    return min([200 if solution[squareNew]==-1 else solution[squareNew] for squareNew in range(square+1,square+7)])+1 
#the min is over all of the dice rolls
for caseIndex in range(numCases):
    solution={1:-1}
    solution[1]=findBest(1,caseIndex,solution)
    if solution[1]<150:
        print solution[1]
    else:
        print -1
