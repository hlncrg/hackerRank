...
'''
#get data
tot,num=[int(i) for i in raw_input().split(' ')]
raw=raw_input().split(' ')
array=[]
for i in range(num):
    array.append(int(raw[i]))
#create table
table = [[0 for x in range(tot+1)] for x in range(num)]
for i in range(num):
    table[i][0]=1#if tot is zero there is one way to make change
for i in range(tot+1):
    table[0][i]=1#if num is zero there is one way to make change
for numIndex in range(num):
    for totIndex in range(1,tot+1):
        #the ways to make change are the number of 
        #ways to make change without coin x and the ways
        #to make change with at least one coin x
        x=table[numIndex-1][totIndex]#ways to make change without coin x
        if totIndex-array[numIndex]<0:
            y=0
        else:
            y=table[numIndex][totIndex-array[numIndex]]
            #ways to make change with at least one coin x
        table[numIndex][totIndex]=x+y    
print table[num-1][tot] 
